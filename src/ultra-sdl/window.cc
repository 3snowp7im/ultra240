...
namespace ultra::sdl {
  ...
  class Window : public ultra::Window {
    ...
    std::unordered_map<const TilesetRange*, std::unique_ptr<TilesetRange>>
    tileset_ranges;
    std::unordered_map<const EntityRange*, std::unique_ptr<EntityRange>>
    entity_ranges;
    std::unordered_map<const BoxRange*, std::unique_ptr<BoxRange>>
    box_ranges;
    std::vector<std::list<Texture*>> map_tile_textures;
    std::vector<TilesetRange> map_tileset_ranges;
  ...
  public:
    Window(
      const char* title,
      SettingsManager& window_settings,
      SettingsManager& control_settings
    );
    ~Window();
    void close();
    bool should_quit();
    ...
    void unload_tilesets(const std::vector<const TilesetRange*>& ranges);
    void load_world(ResourceLoader& loader, const World& world);
    void unload_world();
    ...
    void draw();
  private:
    void render_tiles(size_t layer_start, ssize_t layer_count);
  };
  ...
  static SDL_Window* create_window(
    ...
    if (!window_settings["fullscreen"].IsDefined()) {
    ...
    if (window_settings["x"].IsDefined()) {
      x = window_settings["x"].as<int>();
    }
    if (window_settings["y"].IsDefined()) {
      y = window_settings["y"].as<int>();
    }
    if (window_settings["w"].IsDefined()) {
      w = window_settings["w"].as<int>();
    }
    if (window_settings["h"].IsDefined()) {
      h = window_settings["h"].as<int>();
    }
    if (w == 0 || h == 0) {
      SDL_Rect bounds;
    ...
    if (window_settings["fullscreen"].as<bool>()) {
  ...
  static SDL_Joystick* create_joystick(SettingsManager& controls_settings) {
    if (controls_settings["controller"].IsDefined()) {
      auto controller = controls_settings["controller"].as<std::string>();
      if (controller != "") {
  ...
  TextureList::iterator Window::add_tilesets(
    ResourceLoader& loader,
    const Tileset* const* tilesets,
    size_t tilesets_count,
    Texture::Type type
    ...
    for (int i = 0; i < tilesets_count; i++) {
      util::Image image = util::load_image(
        loader,
        tilesets[i]->source.c_str()
      );
      *texture = {
        .tileset = tilesets[i],
        .type = type,
  ...
  void Window::remove_textures(TextureList::iterator begin, size_t count) {
    if (count > 0) {
      auto it = begin;
  ...
  const Window::TilesetRange* Window::load_tilesets(
    ...
    TilesetRange* range = new TilesetRange {
      .begin = begin,
      .count = tilesets.size(),
      .texture_map = texture_map,
    };
    tileset_ranges.insert({range, std::unique_ptr<TilesetRange>(range)});
    return range;
  }
  ...
  void Window::load_world(ResourceLoader& loader, const World& world) {
    unload_world();
    maps = &world.maps[0];
  ...
  const Window::EntityRange* Window::load_entities(
    ...
    entity_ranges.insert({range, std::unique_ptr<EntityRange>(range)});
    return range;
  }
  ...
  const Window::BoxRange* Window::load_boxes(
    ...
    BoxRange* range = new BoxRange {
      .begin = begin,
      .count = boxes.size()
    };
    box_ranges.insert({
      range,
      std::unique_ptr<BoxRange>(range)
    });
    return range;
  }

  void Window::unload_boxes(
    const std::vector<const BoxRange*> ranges
  ) {
    for (const auto range : ranges) {
      auto end = range->begin;
  ...
  void Window::render_tiles(size_t start_layer_idx, ssize_t layer_count) {
    if (layer_count <= 0) {
      return;
    }
...
namespace ultra {
  ...
  Window* Window::create(
    ...
    return new sdl::Window(title, window_settings, controls_settings);
  }
